...
def rebase_local_addons(addon_path, wow_base):
  ...
  os.renames(addon_path, unique_path)
...
def main(options=[], args=[]):
  ...
  if not options.clean_up: print 'not clean'
  if options.clean_up:
    os.debug('Cleaning up')
    log.debug('cwd %' % os.getcwd())
    os.chdir('..')
    log.debug('cwd %' % os.getcwd())
    try:
      if os.path.exists(new_version_path):
        shutil.copy(os.path.join(new_version_path, 'addongit_up.py'), app_runtime['dir'])
        shutil.rmtree(new_version_path)
    except Exception, e:
      log.error('clean up failed to delete (s): %s\nContinuing.' % (new_version_path, str(e)))
  elif options.update:
    ...
    log.debug('new_version: %s' % new_version_path)
    
    if os.path.exists(new_version_path):
      log.debug('Removing: %s' % new_version_path)
      shutil.rmtree(new_version_path)
    log.debug('Extracting %s to %s' % (tarfile_path, new_version_path))
    ...
    try:
      if app_runtime['type'] == 'python':
        ...
        os.popen('python %s --backup-path=%s --new-version-path=%s --current-version-path=%s --app-start-cmd="%s"' %
                  (app_up, backup_path, new_version_path, app_runtime['dir'], app_runtime['start']))
        log.debug('python %s --backup-path=%s --new-version-path=%s --current-version-path=%s --app-start-cmd="%s"' %
                  (app_up, backup_path, new_version_path, app_runtime['dir'], app_runtime['start']))
        ...
        #          (app_up, backup_path, new_version_path, app_runtime['dir'], app_runtime['start']))
        os.popen('start %s --backup-path=%s --new-version-path=%s --current-version-path=%s --app-start-cmd="%s"' %
                  (app_up, backup_path, new_version_path, app_runtime['dir'], app_runtime['start']))
        log.debug('python %s --backup-path=%s --new-version-path=%s --current-version-path=%s --app-start-cmd="%s"' %
                  (app_up, backup_path, new_version_path, app_runtime['dir'], app_runtime['start']))
    except Exception, e:
      log.error('Unable to upgrade app: %s' % str(e))
    sys.exit()
