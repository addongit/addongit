...
wow_reg = { 'wow_key_path':       'SOFTWARE\\Wow6432Node\\Blizzard Entertainment\\World of Warcraft',
            'key':                None,
            'wow_registry_keys':  0
          }
...
log_settings = {  'path':     os.path.join( app_runtime['dir'], 'log' ),
                  'level':    'info',
                  'app_name': 'addongit'
               }
...
def init_git(addon_path, remote_repo, config_path, branch='master'):
  ...
  log.debug('init_git(%s, %s, %s, %s)' % (  addon_path, 
                                            remote_repo, 
                                            config_path, 
                                            branch
                                         ))
  ...
  # Adds a remote branch for tracking as origin for the local repo
  # and only pulls down that branches objects.
  # git pull/fetch will still on track against this isolated branch.
  if app_settings['uni_branch']:
    log.debug('Uni branch environment detected.')
    os.system('git init')
    git_cmd_remote_add = 'git remote add -t %s -f origin %s' % (branch, remote_repo)
    log.debug(git_cmd_remote_add)
    git_cmd_co = 'git checkout %s' % branch
    log.debug(git_cmd_co)
    os.system(git_cmd_remote_add)
    os.system(git_cmd_co)
  else:
...
def config_defaults():
  ...
  try:
    cfg['uni_branch'] = config.getboolean('local', 'uni_branch')
  except Exception, e:
    log.error(str(e))
    pass
  ...
  app_settings['uni_branch']          = cfg['uni_branch']         if cfg['uni_branch']          is not None else app_settings['uni_branch']
  ...
  app_settings['addon_path']  = os.path.normpath( os.path.join( app_settings['wow_interface_base'], 
                                                                app_settings['addon_base'] 
                                                              ))
  ...
  if wow_reg['key']: 
    ...
    try:
      ...
      if wow_reg['wow_registry_keys'] > 0:
        ...
        app_settings['wow_interface_base']  = os.path.join( wreg.QueryValueEx(  wow_reg['key'], 
                                                                                'InstallPath' 
                                                                             )[0].encode('utf-8'), 
                                                            'Interface' )
...
def main(options=[], args=[]):
  ...
  try:
    log = create_log_handle(  log_settings['path'], 
                              log_settings['level'], 
  ...
  if not os.path.exists( app_settings['addon_path'] ):
    ...
    init_git( app_settings['addon_path'], 
              app_settings['remote_repo'], 
              app_settings['config_path'], 
              app_settings['remote_branch'] )
